...
\begin{frame}
  ...
  \only<5>{
    ...
    Пусть $p \in (p_k^{+0}, p_k^{+1/2})$. Тогда инсайдер использует ставки $k$ и $k+1$ с полными вероятностями 
    \[
      q_k = \frac{p_k^{+1/2} - p}{p_k^{+1/2}-p_k^{+0}}, \: 
      q_{k+1} = 1 - q_k.
    \]
    Для $p \in (p_k^{+1/2}, p_{k+1}^{+0})$ --- аналогично.
