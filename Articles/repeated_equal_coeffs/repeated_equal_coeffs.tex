...
\begin{align*}  
  ...
  H^m(p)
  &\eqdef \min_{k \in J} \left( ph_\infty^H(\tau^k) + (1-p)h_\infty^L(\tau^k) \right) =
    \min_{k \in J} \left( p(m-k)^2 + (1-p)k^2 \right).
\end{align*}
...
Определим стратегию второго игрока $ \tau^* $ следующим образом: при $ p \in (\podd{k-1}, \podd{k+1}] $ второй игрок применяет $ \tau^k $.
Так как 
\[ 
